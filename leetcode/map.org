...
#+INCLUDE: "./twopoints.org"

* stack/heap/queue
#+INCLUDE: "./heap.org"

* linked list
#+INCLUDE: "./linklist.org"
...
Output: "acdb"
   ...
   _*thoughts*_ \\
   - first pass to get the # of each char 
   - second pass, remove the item in the queue 
     - only remove 
       - larger and not last number in the queue 
   - don't forget clean the queue in the end 
...
class Solution(object):
    def removeDuplicateLetters(self, s):
        ...
        for c in s:
            ...
            if c  in current_map and current_map[c] == 1:
                continue
                
            while c_queue and c_queue[-1] >= c and c_map[c_queue[-1]] != 0:
                last =  c_queue.pop()
                current_map[last] = 0
                
            if c_map[c] == 0:
                result = result + ''.join(c_queue) + c
                c_queue = []
                current_map[c] = 1
...
** N-ary tree
   ...
   _*thoughts*_ \\
   - simple queue iterative solution 
...
Given a binary tree, imagine yourself standing on the right side of it, return the values of the nodes you can see ordered from top to bottom.
   ...
   _*thoughts*_ \\
   - what if left is deeper than right ? 
   - need to go through all nodes, but from right to left, fill each depth
...
_*thoughts*_ \\
- straight-forward solution: put link list to an array 
- mid = (l + r)/2  OR mid = (l + r + 1 ) /2 
...
class Solution(object):
    def sortedListToBST(self, head):
        ...
        def build(l, r, vals):
            ...
            mid = int((l + r ) / 2)
            newnode = TreeNode(vals[mid])
            newnode.left = build(l, mid - 1, vals)
            newnode.right = build(mid + 1, r, vals)
            return newnode
...
_*thoughts*_ \\
- list is mutable: in function shouldn't use = to change it, because it's changing the reference 
   - but you can use .append()
- saving path, so we can trace back 
- saving drection( goes left or right ) 
- OR we can save the visited node ( like graph ) 

_*code*_ \\
    #+begin_src python
 class Solution(object):
     def distanceK(self, root, target, K):
         def findbelow(node, depth, distance, result):
             if node == None:
                 return            
             if depth > distance:
                 return            
             ...
             findbelow(node.left, depth + 1, distance, result)
             findbelow(node.right, depth + 1, distance, result)
            
            
         def dfs(node, target, path, direction, result):
             if node == None:
                 return False            
             if node == target:
                 result.append(path + [node])
                 result.append(direction + ['X'])
                 return True            
             if (dfs(node.left, target, path + [node], direction + ['L'], result) or
                 dfs(node.right, target, path + [node], direction + ['R'], result)):
                 return True
             return False

         path_to_target = []
         dfs(root, target, [], [], path_to_target)        
        
         paths = path_to_target[0][::-1]
         directions = path_to_target[1][::-1]
         result = []
         for i, n in enumerate(paths):            
             ...
             if distance == 0:
                 ...
                 # go right 
                 findbelow(n.right, 0, distance - 1, temp)
             elif direction == 'R':
                 findbelow(n.left, 0, distance - 1, temp)
             else:
                 findbelow(n, 0, distance, temp)
...
_*thoughts*_ \\
- typical dfs
- using array as map to save visited state
- [[Course Schedule]] is dfs too, but it's black grey dfs  
...
class Solution(object):
    def findCircleNum(self, M):
        ...
        def dfs(node, visited):
            ...
            for idx, know in enumerate(M[node]):
                if idx == node:
                    continue
                if know == 1: # it's neighbor
                    if visited[idx] == 0:
                        dfs(idx, visited)
        count = 0
        for idx, isvisited in enumerate(visited):
            if isvisited == 0: # not visited
                count += 1
                dfs(idx, visited)
...
This works because ('X', 'Y', 'D'), ('Y', 'Z', 'E'), and ('D', 'E', 'A') are allowed triples.
   ...
   _*thoughts*_ \\
   - should be recursive solution 
   - get all possiblities of the next level 
     - if possible, dfs to next level 
     - exit condition: len(current_level) == 1
...
class Solution(object):
    def pyramidTransition(self, bottom, allowed):
        ...
        def allpossible(current, allowed):
            if len(current) < 2:
                return ['']
            ...
            for allow in allowed:
                if allow.startswith(first):
                    top = allow[2]                    
                    for possible in allpossible(current[1:], allowed):
                        result.append(top + possible)
            return result       
            
        def dfs(current, allowed):
            if len(current) == 1:
                return True
            
            for possible in allpossible(current, allowed):
                if dfs(possible, allowed):
                    return True
                
            return False
        
        return dfs(bottom, 1, allowed)            
   #+end_src
...
class Solution(object):
    def longestIncreasingPath(self, matrix):
        ...
        r_len = len(matrix)
        c_len = len(matrix[0])
        state = [[None for _ in range(c_len)] for _ in range(r_len)]
        
        def dfs(i, j, matrix, level, state):
            ...
            for neighbor in neighbors:
                x, y = neighbor
                if 0 <= x and x < r_len and 0 <= y and y < c_len:
                    new_val = matrix[x][y]
                    if new_val > curr:
                        
                        tmp = max(tmp, dfs(x, y, matrix, level + 1, state))
...
** sort colors
   ...
   - 3 points, i, k, j
     - note i <= k <= j, use it as exit condition 
   - think of exit condition first, no harm to do more strict exit condition
...
class Solution(object):
    def sortColors(self, nums):
        ...
        j = len(nums) - 1
        k = 0
        while k < len(nums) and k <= j and k >=i:
            if nums[k] == 0:
...
** Search in Rotated Sorted Array                                       :ERROR:
#+begin_src python
class Solution(object):
    def search(self, nums, target):
...
** 74. search a 2d matrix
   ...
   _*thoughts*_ \\
   - beware the last item in list is len-1
   - python class method first signature is self
...
class Solution(object):
    def searchMatrix(self, matrix, target):
        ...
        return self.find(matrix, 0, 0, row_len-1, col_len-1, target )
        
    def find(self, matrix, i1, j1, i2, j2, target):
        if i1 > i2 or j1 > j2:
            return False
        ...
        if target > matrix[imid][jmid]:
            return self.find(matrix, i1, jmid + 1, i2, j2, target) or self.find(matrix, imid+1, j1, i2, j2, target )
        
        else:
            return self.find(matrix, i1, j1, imid - 1, j2, target) or self.find(matrix, i1, j1, i2, jmid - 1, target)
...
* TODO divide and conquer 
- binary search is one kind of divide & conquer
#+begin_src 
DAC(P)
  if small(P) return SP(P)
  else p1, p2, .... pk = divide(P)
       combine(p1, p2 .... pk) 
#+end_src
...
** 307. Range Sum Query - Mutable
   ...
   _*thoughts*_ \\
   - segmented tree 
   - use array to represent a full binary tree
     - level = log2(len(nums))
   - don't forget to update nums after updating tree 
...
class NumArray(object):
    def __init__(self, nums):
        ...
        import math
        self.nums = nums
        x = int(math.ceil(math.log(len(nums),2)))
        self.tree = [None] * (2 * 2**x + 2)
        self.tree[0] = self.buildSTree(nums, 0, len(nums)-1, 0)
        

    def update(self, i, val):
        diff = val - self.nums[i]
        self.updateSTree(diff, i, 0, len(self.nums)-1, 0)
        self.nums[i] = val
        

    def sumRange(self, i, j):
        result =  self.getsum(i, j, 0, len(self.nums)-1, 0)
        return result
                
        
    def buildSTree(self, nums, s, e, i):
        if not nums:
            return None
        ...
        mid = (s+e)/2
        self.tree[i] = self.buildSTree(nums, s, mid, 2*i + 1) + self.buildSTree(nums, mid+1, e, 2*i + 2)
        return self.tree[i]
    
    def updateSTree(self, diff, index, s, e, i):  
        self.tree[i] += diff
        if s == e and s == index:
            return
        ...
        mid = (s + e)/2
        if index <= mid: # left
            self.updateSTree(diff, index, s, mid, i*2 + 1)
        else:
            self.updateSTree(diff, index, mid+1, e, i*2 + 2)
        
    def getsum(self, i, j, s, e, ti):
        if i <= s and e <= j:
            return self.tree[ti]
        ...
        mid = (s+e)/2
        return self.getsum(i, j, s, mid, 2*ti + 1) + self.getsum(i, j, mid+1, e, 2*ti + 2)                
   #+end_src
